	...
	*/
	...
	protected static void doCreateUser() {
		...
		if (!userNameError.isEmpty()) {
			ViewNewAccount.alertUsernameError.setContentText("Invalid username: " + userNameError);
		...
		if (!password1.equals(password2)) {
			...
			ViewNewAccount.alertPasswordMatchError.setContentText("The two passwords must match. Please try again!");
			ViewNewAccount.alertPasswordMatchError.showAndWait();
			return;
		}
		// Validate password requirements
		String passwordError = Model.evaluatePassword(password1);
			if (!passwordError.isEmpty()) {
				ViewNewAccount.text_Password1.setText("");
				ViewNewAccount.text_Password2.setText("");
				ViewNewAccount.alertPasswordError.setContentText("Password does not meet requirements: " + passwordError);
