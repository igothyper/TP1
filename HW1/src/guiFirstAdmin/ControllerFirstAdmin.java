...
public class ControllerFirstAdmin {
	...
	*/
	...
	protected static void doSetupAdmin(Stage ps, int r) {
	    ...
	    String userNameError = userNameRecognizerTestbed.UserNameRecognizer.checkForValidUserName(adminUsername);
	    if (!userNameError.isEmpty()) {
	        ViewFirstAdmin.label_PasswordsDoNotMatch.setText(""); // Clear any password error
	        ViewFirstAdmin.label_UserNameInvalid.setText("Invalid username: " + userNameError);
	        return;
	    } else {
	        ViewFirstAdmin.label_UserNameInvalid.setText(""); // Clear previous username error
	    }
		// Make sure the two passwords are the same
		if (adminPassword1.compareTo(adminPassword2) == 0) {
	        // Evaluate the password using your FSM
	        String passwordError = passwordPopUpWindow.Model.evaluatePassword(adminPassword1);
	        if (!passwordError.isEmpty()) {
	            // Show the error to the user and clear the password fields
	            ViewFirstAdmin.text_AdminPassword1.setText("");
	            ViewFirstAdmin.text_AdminPassword2.setText("");
	            ViewFirstAdmin.label_PasswordsDoNotMatch.setText(
	                "Password does not meet requirements: " + passwordError
	            );
	            return; // Don't continue to register the user
	        }
